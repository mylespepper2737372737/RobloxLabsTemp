...
(async () => {
	...
	const [didInsert, errMessage] = await tbl0.InsertValuesIntoTable([
	...
	const [didGetRows, reason, result] = await tbl0.GetAllRowsByKey('Id', 100);
	if (!didGetRows) throw reason;
	console.log(result);
	const [didGetRows2, reason2, result2] = await tbl0.GetAllRows(100);
	if (!didGetRows2) throw reason2;
	console.log(result2);
	const [didGetRows3, reason3, result3] = await tbl0.GetAllRowsByKeys(['Test'], 100);
	if (!didGetRows3) throw reason3;
	console.log(result3);
	const [didGetRows4, reason4, result4] = await tbl0.GetFromRowsByKeyConditional('Test', {
		Key: 'Id',
