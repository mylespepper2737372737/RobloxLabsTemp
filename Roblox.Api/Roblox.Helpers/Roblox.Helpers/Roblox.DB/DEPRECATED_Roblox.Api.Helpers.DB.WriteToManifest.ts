...
export const WriteToManifest = (
	...
	let user;
	try {
		user = JSON.parse(filestream.readFileSync(_dirname + `\\Roblox.Manifest\\users\\${userId}.json`, { encoding: 'utf-8' }));
	} catch (e) {
		return e;
	}
	if (user) {
	...
	return filestream.writeFileSync(_dirname + `\\Roblox.Manifest\\users\\${userId}.json`, JSON.stringify(user, undefined, 4), {
		encoding: 'utf-8',
	});
};
