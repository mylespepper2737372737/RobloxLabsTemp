...
import { GetPersistentStoreForUniverse } from '../Roblox.PersistentDataStores.GetHelpers/Roblox.PersistentDataStores.GetHelpers.GetPersistentStoreForUniverse';
...
export const PurgeUniversePersistentStore = (universeId: number, storeName: string, isSorted: boolean = false): Promise<boolean> => {
	return new Promise<boolean>(async (resumefunction) => {
		const dir = _dirname + '\\Roblox.Manifest\\persistence\\' + universeId;
