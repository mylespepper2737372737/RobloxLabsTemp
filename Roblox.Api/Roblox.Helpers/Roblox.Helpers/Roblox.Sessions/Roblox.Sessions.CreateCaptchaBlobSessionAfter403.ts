...
export const CreateCaptchaBlobSessionAfter403 = (response: Response, captchaBLOB: string, ip: string) => {
	...
	filestream.writeFileSync(_dirname + `\\Roblox.Manifest\\sessions\\${captchaBLOB}.json`, JSON.stringify(dataToRefer), {
		encoding: 'ascii',
	});
	setTimeout(() => {
		try {
			filestream.unlinkSync(_dirname + `\\Roblox.Manifest\\sessions\\${captchaBLOB}.json`);
		} catch {
