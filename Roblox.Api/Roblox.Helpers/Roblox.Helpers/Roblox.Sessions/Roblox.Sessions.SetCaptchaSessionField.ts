...
export const SetCaptchaSessionField = (
	...
	let session;
	try {
		session = JSON.parse(filestream.readFileSync(_dirname + `\\Roblox.Manifest\\sessions\\${sessionId}.json`, { encoding: 'utf-8' }));
	} catch (e) {
		return e;
	}
	if (session) {
	...
	return filestream.writeFileSync(_dirname + `\\Roblox.Manifest\\sessions\\${sessionId}.json`, JSON.stringify(session, undefined, 4), {
		encoding: 'utf-8',
	});
};
