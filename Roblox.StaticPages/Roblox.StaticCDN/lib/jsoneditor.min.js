!(function () {
	...
	!(function () {
		...
		return (
			...
			(t.extend = function (t) {
				...
				for (var o in t)
					n[o] =
						'function' == typeof t[o] && 'function' == typeof r[o] && i.test(t[o])
							? (function (t, e) {
									return function () {
										var i = this._super;
										this._super = r[t];
							...
							: t[o];
				return (s.prototype = n), (s.prototype.constructor = s), (s.extend = arguments.callee), s;
			}),
			t
		);
	})(),
	...
	var e = function (t) {
			return (
				!('object' != typeof t || t.nodeType || (null !== t && t === t.window)) &&
				!(t.constructor && !Object.prototype.hasOwnProperty.call(t.constructor.prototype, 'isPrototypeOf'))
			);
		},
		i = function (t) {
			var s, r, n;
	...
	(n.prototype = {
		...
		init: function () {
			...
			i && (this.iconlib = new i()),
				...
				this._loadExternalRefs(this.schema, function () {
					...
					(t.root = t.createEditor(e, { jsoneditor: t, schema: t.schema, required: !0, container: t.root_container })),
						t.root.preBuild(),
						t.root.build(),
						t.root.postBuild(),
		...
		_data: function (t, e, i) {
			if (3 !== arguments.length)
				return t.hasAttribute('data-jsoneditor-' + e) ? this.__data[t.getAttribute('data-jsoneditor-' + e)] : null;
			var s;
			...
			if (((e = e || '#/definitions/'), t.definitions))
				for (var i in t.definitions)
					t.definitions.hasOwnProperty(i) &&
						...
						t.definitions[i].definitions && this._getDefinitions(t.definitions[i], e + i + '/definitions/'));
		},
						...
						(s.onreadystatechange = function () {
							if (4 == s.readyState) {
								...
								try {
									...
									throw (window.console.log(h), 'Failed to parse external ref ' + t);
								}
					...
					'undefined' != typeof e[t]
						? 'required' === t && 'object' == typeof i && Array.isArray(i)
							...
							: ('string' == typeof i && (i = [i]),
							  ...
							  1 === n.type.length && 'string' == typeof n.type[0] && (n.type = n.type[0]))
						: (n[t] = i);
				}),
				s(e, function (e, i) {
		...
		(n.defaults = { themes: {}, templates: {}, iconlibs: {}, editors: {}, languages: {}, resolvers: [], custom_validators: [] }),
			...
			_validateSchema: function (t, e, r) {
				...
				if (((r = r || 'root'), (t = i({}, this.jsoneditor.expandRefs(t))), t.required && t.required === !0)) {
					if ('undefined' == typeof e)
						return d.push({ path: r, property: 'required', message: this.translate('error_notset') }), d;
				} else if ('undefined' == typeof e) {
					if (!this.jsoneditor.options.required_by_default) return d;
					d.push({ path: r, property: 'required', message: this.translate('error_notset') });
				}
				if (t['enum']) {
					for (o = !1, a = 0; a < t['enum'].length; a++) u === JSON.stringify(t['enum'][a]) && (o = !0);
					o || d.push({ path: r, property: 'enum', message: this.translate('error_enum') });
				}
				if (t['extends']) for (a = 0; a < t['extends'].length; a++) d = d.concat(this._validateSchema(t['extends'][a], e, r));
				if (t.allOf) for (a = 0; a < t.allOf.length; a++) d = d.concat(this._validateSchema(t.allOf[a], e, r));
				if (t.anyOf) {
					...
					o || d.push({ path: r, property: 'anyOf', message: this.translate('error_anyOf') });
				}
				if (t.oneOf) {
					o = 0;
					var p = [];
					for (a = 0; a < t.oneOf.length; a++) {
						...
						for (c.length || o++, h = 0; h < c.length; h++) c[h].path = r + '.oneOf[' + a + ']' + c[h].path.substr(r.length);
						p = p.concat(c);
					}
					1 !== o && (d.push({ path: r, property: 'oneOf', message: this.translate('error_oneOf', [o]) }), (d = d.concat(p)));
				}
				if (
					(t.not &&
					...
					if (Array.isArray(t.type)) {
						...
						o || d.push({ path: r, property: 'type', message: this.translate('error_type_union') });
					} else
						this._checkType(t.type, e) ||
							d.push({ path: r, property: 'type', message: this.translate('error_type', [t.type]) });
				if (t.disallow)
					if (Array.isArray(t.disallow)) {
						...
						o || d.push({ path: r, property: 'disallow', message: this.translate('error_disallow_union') });
					} else
						this._checkType(t.disallow, e) &&
							d.push({ path: r, property: 'disallow', message: this.translate('error_disallow', [t.disallow]) });
				if ('number' == typeof e)
					(t.multipleOf || t.divisibleBy) &&
						...
						o !== Math.floor(o) &&
							d.push({
								...
								message: this.translate('error_multipleOf', [t.multipleOf || t.divisibleBy]),
							})),
						t.hasOwnProperty('maximum') &&
							(t.exclusiveMaximum && e >= t.maximum
								? d.push({ path: r, property: 'maximum', message: this.translate('error_maximum_excl', [t.maximum]) })
								: !t.exclusiveMaximum &&
								  e > t.maximum &&
								  d.push({ path: r, property: 'maximum', message: this.translate('error_maximum_incl', [t.maximum]) })),
						t.hasOwnProperty('minimum') &&
							(t.exclusiveMinimum && e <= t.minimum
								? d.push({ path: r, property: 'minimum', message: this.translate('error_minimum_excl', [t.minimum]) })
								: !t.exclusiveMinimum &&
								  e < t.minimum &&
								  d.push({ path: r, property: 'minimum', message: this.translate('error_minimum_incl', [t.minimum]) }));
				else if ('string' == typeof e)
					t.maxLength &&
						(e + '').length > t.maxLength &&
						d.push({ path: r, property: 'maxLength', message: this.translate('error_maxLength', [t.maxLength]) }),
						t.minLength &&
						...
						t.pattern &&
							(new RegExp(t.pattern).test(e) ||
								d.push({ path: r, property: 'pattern', message: this.translate('error_pattern') }));
					...
					if (t.items)
						...
						else for (a = 0; a < e.length; a++) d = d.concat(this._validateSchema(t.items, e[a], r + '.' + a));
					if (
						(t.maxItems &&
							e.length > t.maxItems &&
							d.push({ path: r, property: 'maxItems', message: this.translate('error_maxItems', [t.maxItems]) }),
						t.minItems &&
							e.length < t.minItems &&
							d.push({ path: r, property: 'minItems', message: this.translate('error_minItems', [t.minItems]) }),
						t.uniqueItems)
					) {
						var m = {};
						for (a = 0; a < e.length; a++) {
							if (((o = JSON.stringify(e[a])), m[o])) {
								d.push({ path: r, property: 'uniqueItems', message: this.translate('error_uniqueItems') });
								break;
							}
							m[o] = !0;
						}
					...
					if (t.required && Array.isArray(t.required))
						for (a = 0; a < t.required.length; a++)
							'undefined' == typeof e[t.required[a]] &&
								d.push({ path: r, property: 'required', message: this.translate('error_required', [t.required[a]]) });
					var f = {};
					if (t.properties)
						for (a in t.properties)
							t.properties.hasOwnProperty(a) &&
								((f[a] = !0), (d = d.concat(this._validateSchema(t.properties[a], e[a], r + '.' + a))));
						...
						for (a in t.patternProperties)
							if (t.patternProperties.hasOwnProperty(a)) {
								...
								for (h in e)
									e.hasOwnProperty(h) &&
										g.test(h) &&
										((f[h] = !0), (d = d.concat(this._validateSchema(t.patternProperties[a], e[h], r + '.' + h))));
							}
					if (
									...
									for (h = 0; h < t.dependencies[a].length; h++)
										'undefined' == typeof e[t.dependencies[a][h]] &&
											d.push({
												...
												message: this.translate('error_dependency', [t.dependencies[a][h]]),
											});
		...
		(n.AbstractEditor = t.extend({
			...
			init: function (t) {
				(this.jsoneditor = t.jsoneditor),
					...
					(this.formname = t.formname || this.path.replace(/\.([^.]+)/g, '[$1]')),
						...
						if (this.schema.watch.hasOwnProperty(o)) {
							if (
								...
								Array.isArray(e)
									...
									: ((i = e.split('.')),
									  t.theme.closest(t.container, '[data-schemaid="' + i[0] + '"]') || i.unshift('#')),
								(s = i.shift()),
								'#' === s && (s = t.jsoneditor.schema.id || 'root'),
								(r = t.theme.closest(t.container, '[data-schemaid="' + s + '"]')),
								!r)
							)
			...
			addLinks: function () {
				if (
					!this.no_link_holder &&
					((this.link_holder = this.theme.getLinksHolder()), this.container.appendChild(this.link_holder), this.schema.links)
				)
				...
				if ('image' === r) {
					...
					this.theme.createImageLink(e, i, o),
						this.link_watchers.push(function (e) {
							var s = n(e);
							i.setAttribute('href', s), i.setAttribute('title', t.rel || s), o.setAttribute('src', s);
						});
					...
					a.setAttribute('controls', 'controls'),
						...
						this.link_watchers.push(function (e) {
							var s = n(e);
							i.setAttribute('href', s), (i.textContent = t.rel || s), a.setAttribute('src', s);
						});
				} else
			...
			getDisplayText: function (t) {
				...
				s(t, function (t, e) {
					e.title && ((i[e.title] = i[e.title] || 0), i[e.title]++),
						e.description && ((i[e.description] = i[e.description] || 0), i[e.description]++),
		...
		(n.defaults.editors.string = n.AbstractEditor.extend({
			...
			getNumColumns: function () {
				...
				return (
					(t = 'textarea' === this.input_type ? 6 : ['text', 'email'].indexOf(this.input_type) >= 0 ? 4 : 2),
			...
			build: function () {
				...
				if (
					...
					this.schema.description && (this.description = this.theme.getFormInputDescription(this.schema.description)),
					...
					if ('textarea' === this.format) (this.input_type = 'textarea'), (this.input = this.theme.getTextareaInput());
						...
						this.schema.multipleOf &&
							...
							i % this.schema.multipleOf && (i = Math.floor(i / this.schema.multipleOf) * this.schema.multipleOf),
				...
				'undefined' != typeof this.schema.maxLength && this.input.setAttribute('maxlength', this.schema.maxLength),
			...
			afterInputReady: function () {
				...
				if (this.source_code)
					if (
						...
						(t = i(
							{},
							{ plugins: 'html' === e.input_type ? 'xhtml' : 'bbcode', emoticonsEnabled: !1, width: '100%', height: 300 },
							n.plugins.sceditor,
							e.options.sceditor_options || {},
						)),
							window.jQuery(e.input).sceditor(t),
							(e.sceditor_instance = window.jQuery(e.input).sceditor('instance')),
							e.sceditor_instance.blur(function () {
								var t = window.jQuery('<div>' + e.sceditor_instance.val() + '</div>');
								window.jQuery('#sceditor-start-marker,#sceditor-end-marker,.sceditor-nlf', t).remove(),
						...
						(this.epiceditor_container = document.createElement('div')),
							...
							(t = i({}, n.plugins.epiceditor, { container: this.epiceditor_container, clientSideStorage: !1 })),
						...
						('cpp' === s || 'c++' === s || 'c' === s) && (s = 'c_cpp'),
							...
							n.plugins.ace.theme && this.ace_editor.setTheme('ace/theme/' + n.plugins.ace.theme),
		...
		(n.defaults.editors.object = n.AbstractEditor.extend({
			...
			register: function () {
				if ((this._super(), this.editors)) for (var t in this.editors) this.editors.hasOwnProperty(t) && this.editors[t].register();
			},
			unregister: function () {
						...
						for (
							s(this.property_order, function (t, e) {
								...
								if (!s.property_removed) {
									for (
										var r = !1,
											o = s.options.hidden ? 0 : s.options.grid_columns || s.getNumColumns(),
											a = s.options.hidden ? 0 : s.container.offsetHeight,
											h = 0;
										h < n.length;
										h++
									)
										n[h].width + o <= 12 && (!a || (0.5 * n[h].minh < a && 2 * n[h].maxh > a)) && (r = h);
									r === !1 && (n.push({ width: 0, minh: 999999, maxh: 0, editors: [] }), (r = n.length - 1)),
							...
							if (n[t].width < 12) {
								...
								for (e = 0; e < n[t].editors.length; e++)
									o === !1 ? (o = e) : n[t].editors[e].width > n[t].editors[o].width && (o = e),
						...
						for (this.layout = JSON.stringify(n), r = document.createElement('div'), t = 0; t < n.length; t++) {
							...
							for (r.appendChild(h), e = 0; e < n[t].editors.length; e++) {
								...
								d.options.hidden
									...
									: this.theme.setGridColumnSize(d.container, n[t].editors[e].width),
					...
					(this.defaultProperties = this.schema.defaultProperties || Object.keys(this.schema.properties)),
						...
						s(this.defaultProperties, function (e, i) {
							t.addObjectProperty(i, !0),
								t.editors[i] &&
									((t.minwidth = Math.max(t.minwidth, t.editors[i].options.grid_columns || t.editors[i].getNumColumns())),
									(t.maxwidth += t.editors[i].options.grid_columns || t.editors[i].getNumColumns()));
						});
				}
				(this.property_order = Object.keys(this.editors)),
				...
				if (this.options.table_row)
					(this.editor_holder = this.container),
						s(this.editors, function (e, i) {
							...
							t.editor_holder.appendChild(s),
								...
								t.editors[e].options.input_width && (s.style.width = t.editors[e].options.input_width);
						});
				else {
					if (this.options.table) throw 'Not supported yet';
						...
						this.addproperty_add.addEventListener('click', function (e) {
							if ((e.preventDefault(), e.stopPropagation(), t.addproperty_input.value)) {
								...
								t.addObjectProperty(t.addproperty_input.value),
									t.editors[t.addproperty_input.value] && t.editors[t.addproperty_input.value].disable(),
									t.onChange(!0);
							}
						}),
					...
					(e.style.clear = 'both'),
							...
							e.preventDefault(),
								...
								t.collapsed
									? ((t.editor_holder.style.display = ''),
									  (t.collapsed = !1),
									  t.setButtonText(t.toggle_button, '', 'collapse', 'Collapse'))
						...
						this.schema.options && 'undefined' != typeof this.schema.options.disable_edit_json
							? this.schema.options.disable_edit_json && (this.editjson_button.style.display = 'none')
							: this.jsoneditor.options.disable_edit_json && (this.editjson_button.style.display = 'none'),
			...
			insertPropertyControlUsingPropertyOrder: function (t, e, i) {
				var s;
				this.schema.properties[t] && (s = this.schema.properties[t].propertyOrder),
			...
			addPropertyCheckbox: function (t) {
				...
				return (
					...
					(e.style.width = 'auto'),
					(s = this.schema.properties[t] && this.schema.properties[t].title ? this.schema.properties[t].title : t),
					...
					(r.style.height = 'auto'),
					this.insertPropertyControlUsingPropertyOrder(t, r, this.addproperty_list),
			...
			hideAddProperty: function () {
				this.addproperty_holder &&
					...
					((this.addproperty_holder.style.display = 'none'), this.enable(), (this.adding_property = !1));
			},
			...
			refreshAddProperties: function () {
				...
				(e =
					...
					!('undefined' != typeof this.schema.maxProperties && s >= this.schema.maxProperties)),
				...
				for (t in this.cached_editors)
					this.cached_editors.hasOwnProperty(t) &&
						...
						this.isRequired(this.cached_editors[t]) && t in this.editors && (this.addproperty_checkboxes[t].disabled = !0),
						'undefined' != typeof this.schema.minProperties && s <= this.schema.minProperties
							? ((this.addproperty_checkboxes[t].disabled = this.addproperty_checkboxes[t].checked),
				...
				for (t in this.schema.properties)
					this.schema.properties.hasOwnProperty(t) && (this.cached_editors[t] || ((r = !0), this.addPropertyCheckbox(t)));
				r
					? this.canHaveAdditionalProperties()
						? e
			...
			showValidationErrors: function (t) {
				...
				s(t, function (t, s) {
					...
					this.options.table_row &&
						(i.length ? this.theme.addTableRowError(this.container) : this.theme.removeTableRowError(this.container)),
		...
		(n.defaults.editors.array = n.AbstractEditor.extend({
			...
			register: function () {
				if ((this._super(), this.rows)) for (var t = 0; t < this.rows.length; t++) this.rows[t].register();
			},
			unregister: function () {
				if ((this._super(), this.rows)) for (var t = 0; t < this.rows.length; t++) this.rows[t].unregister();
			},
			getNumColumns: function () {
			...
			enable: function () {
				if (
					...
					for (var t = 0; t < this.rows.length; t++)
						this.rows[t].enable(),
							this.rows[t].moveup_button && (this.rows[t].moveup_button.disabled = !1),
							this.rows[t].movedown_button && (this.rows[t].movedown_button.disabled = !1),
							this.rows[t].delete_button && (this.rows[t].delete_button.disabled = !1);
				this._super();
			},
			disable: function () {
				if (
					...
					for (var t = 0; t < this.rows.length; t++)
						this.rows[t].disable(),
							...
							this.rows[t].delete_button && (this.rows[t].delete_button.disabled = !0);
				this._super();
			},
			preBuild: function () {
				this._super(),
			...
			getElementEditor: function (t) {
				...
				return (
					...
					n.build(),
					n.postBuild(),
					n.title_controls || ((n.array_controls = this.theme.getButtonHolder()), s.appendChild(n.array_controls)),
					n
				);
			},
			destroy: function () {
				this.empty(!0),
					this.title && this.title.parentNode && this.title.parentNode.removeChild(this.title),
					this.description && this.description.parentNode && this.description.parentNode.removeChild(this.description),
					this.row_holder && this.row_holder.parentNode && this.row_holder.parentNode.removeChild(this.row_holder),
					this.controls && this.controls.parentNode && this.controls.parentNode.removeChild(this.controls),
			...
			destroyRow: function (t, e) {
				...
				e
					...
					: (t.tab && (t.tab.style.display = 'none'), (i.style.display = 'none'), t.unregister());
			},
			getMax: function () {
				...
				s(this.rows, function (i, s) {
					s.tab &&
						(t
							...
							: (e.theme.markTabInactive(s.tab), (s.container.style.display = 'none')));
				});
			},
			setValue: function (t, e) {
			...
			refreshValue: function (t) {
				...
				if (
					...
					s(this.rows, function (t, i) {
						i.movedown_button &&
							(t === e.rows.length - 1 ? (i.movedown_button.style.display = 'none') : (i.movedown_button.style.display = '')),
							i.delete_button && (r ? (i.delete_button.style.display = 'none') : (i.delete_button.style.display = '')),
							(e.value[t] = i.getValue());
					});
					var n = !1;
					this.value.length
						? 1 === this.value.length
							? ((this.remove_all_rows_button.style.display = 'none'),
							  r || this.hide_delete_buttons
									...
									: ((this.delete_last_row_button.style.display = ''), (n = !0)))
							: r || this.hide_delete_buttons
							? ((this.delete_last_row_button.style.display = 'none'), (this.remove_all_rows_button.style.display = 'none'))
							: ((this.delete_last_row_button.style.display = ''), (this.remove_all_rows_button.style.display = ''), (n = !0))
						: ((this.delete_last_row_button.style.display = 'none'), (this.remove_all_rows_button.style.display = 'none')),
						(this.getMax() && this.getMax() <= this.rows.length) || this.hide_add_button
							? (this.add_row_button.style.display = 'none')
							: ((this.add_row_button.style.display = ''), (n = !0)),
						!this.collapsed && n ? (this.controls.style.display = 'inline-block') : (this.controls.style.display = 'none');
				}
			},
			addRow: function (t, e) {
				var i = this,
				...
				i.hide_delete_buttons ||
					((i.rows[r].delete_button = this.getButton(i.getItemTitle(), 'delete', 'Delete ' + i.getItemTitle())),
						...
						s(r, function (t, s) {
							return t === e
								? void (
										...
										(i.rows[t + 1] ? (o = i.rows[t].tab) : t && (o = i.rows[t - 1].tab))
								  )
								: void n.push(s);
						}),
							i.setValue(n),
					...
					r &&
						!i.hide_move_buttons &&
						((i.rows[r].moveup_button = this.getButton('', 'moveup', 'Move up')),
					...
					i.hide_move_buttons ||
						((i.rows[r].movedown_button = this.getButton('', 'movedown', 'Move down')),
			...
			addControls: function () {
				...
				this.toggle_button.addEventListener('click', function (s) {
					...
					this.delete_last_row_button.addEventListener('click', function (e) {
						...
						var i = t.getValue(),
							s = null;
						t.rows.length > 1 && t.rows[t.rows.length - 1].tab === t.active_tab && (s = t.rows[t.rows.length - 2].tab),
							i.pop(),
							t.setValue(i),
		...
		(n.defaults.editors.table = n.defaults.editors.array.extend({
			...
			setValue: function (t, e) {
				if (((t = t || []), this.schema.minItems)) for (; t.length < this.schema.minItems; ) t.push(this.getItemDefault());
				...
				if (i !== this.serialized) {
					...
					for (var o = t.length; o < n.rows.length; o++) {
						...
						n.item_has_child_editors || n.rows[o].row.parentNode.removeChild(n.rows[o].row),
							n.rows[o].destroy(),
			...
			refreshRowButtons: function () {
				...
				s(this.rows, function (s, r) {
					r.movedown_button &&
						...
						r.delete_button &&
							(e ? (r.delete_button.style.display = 'none') : ((i = !0), (r.delete_button.style.display = ''))),
						r.moveup_button && (i = !0);
				}),
					s(this.rows, function (t, e) {
				...
				this.value.length
					...
					r ? (this.controls.style.display = '') : (this.controls.style.display = 'none');
			},
			refreshValue: function () {
				var t = this;
				(this.value = []),
			...
			addRow: function (t) {
				...
				this.hide_delete_buttons ||
					...
					i &&
						...
						((e.rows[i].moveup_button = this.getButton('', 'moveup', 'Move up')),
					...
					this.hide_move_buttons ||
						((e.rows[i].movedown_button = this.getButton('', 'movedown', 'Move down')),
					...
					this.title_controls &&
						...
						this.toggle_button.addEventListener('click', function (e) {
							e.preventDefault(),
								e.stopPropagation(),
								t.collapsed
									? ((t.collapsed = !1), (t.panel.style.display = ''), t.setButtonText(this, '', 'collapse', 'Collapse'))
									: ((t.collapsed = !0), (t.panel.style.display = 'none'), t.setButtonText(this, '', 'expand', 'Expand'));
						}),
						this.options.collapsed && r(this.toggle_button, 'click'),
						this.schema.options && 'undefined' != typeof this.schema.options.disable_collapse
							? this.schema.options.disable_collapse && (this.toggle_button.style.display = 'none')
		...
		(n.defaults.editors.multiple = n.AbstractEditor.extend({
			...
			switchEditor: function (t) {
				...
				s(e.editors, function (t, s) {
					s &&
						(e.type === t
							? (e.keep_values && s.setValue(i, !0), (s.container.style.display = ''))
							: (s.container.style.display = 'none'));
				}),
					e.refreshValue(),
					e.refreshHeaderText();
			...
			preBuild: function () {
				if (
					...
					'undefined' != typeof this.options.keep_oneof_values && (this.keep_values = this.options.keep_oneof_values),
					this.schema.oneOf)
				)
					(this.oneOf = !0), (this.types = this.schema.oneOf), s(this.types, function (t, e) {}), delete this.schema.oneOf;
				else {
		...
		(n.defaults.editors['enum'] = n.AbstractEditor.extend({
			...
			build: function () {
				...
				for (var t = this, e = 0; e < this['enum'].length; e++)
					(this.select_options[e] = this.options.enum_titles[e] || 'Value ' + (e + 1)),
			...
			getHTML: function (t) {
				...
				if ('object' == typeof t) {
					...
					return (
						...
						(i = Array.isArray(t)
							? '<ol>' + i + '</ol>'
							: "<ul style='margin-top:0;margin-bottom:0;padding-top:0;padding-bottom:0;'>" + i + '</ul>')
					);
				}
		...
		(n.defaults.editors.select = n.AbstractEditor.extend({
			setValue: function (t, e) {
				...
				this.enum_values.indexOf(i) < 0 && (i = this.enum_values[0]),
					this.value !== i &&
						((this.input.value = this.enum_options[this.enum_values.indexOf(i)]),
			...
			getNumColumns: function () {
				...
				for (var t = this.getTitle().length, e = 0; e < this.enum_options.length; e++)
			...
			preBuild: function () {
				...
				if (
					...
					s(this.schema['enum'], function (i, s) {
						(t.enum_options[i] = '' + s), (t.enum_display[i] = '' + (e[i] || s)), (t.enum_values[i] = t.typecast(s));
					}),
						this.isRequired() ||
							(t.enum_display.unshift(' '), t.enum_options.unshift('undefined'), t.enum_values.unshift(void 0));
				} else if ('boolean' === this.schema.type)
					(t.enum_display = (this.schema.options && this.schema.options.enum_titles) || ['true', 'false']),
						(t.enum_options = ['1', '']),
						(t.enum_values = [!0, !1]),
						this.isRequired() ||
							(t.enum_display.unshift(' '), t.enum_options.unshift('undefined'), t.enum_values.unshift(void 0));
					...
					if (
						...
						this.schema.enumValue
							? (this.enumSource = [{ source: this.schema.enumSource, value: this.schema.enumValue }])
							: (this.enumSource = [{ source: this.schema.enumSource }]);
					for (a = 0; a < this.enumSource.length; a++)
						this.enumSource[a].value &&
							(this.enumSource[a].value = this.jsoneditor.compileTemplate(this.enumSource[a].value, this.template_engine)),
			...
			setupSelect2: function () {
				if (
					...
					this.schema.options && this.schema.options.select2_options && (t = i(t, this.schema.options.select2_options)),
					...
					for (var i = [], s = [], r = 0; r < this.enumSource.length; r++)
						if (Array.isArray(this.enumSource[r])) (i = i.concat(this.enumSource[r])), (s = s.concat(this.enumSource[r]));
							...
							if ((n = Array.isArray(this.enumSource[r].source) ? this.enumSource[r].source : t[this.enumSource[r].source])) {
								if (
									(this.enumSource[r].slice && (n = Array.prototype.slice.apply(n, this.enumSource[r].slice)),
									this.enumSource[r].filter)
								) {
									var o = [];
									for (e = 0; e < n.length; e++)
										this.enumSource[r].filter({ i: e, item: n[e], watched: t }) && o.push(n[e]);
									n = o;
								}
								var a = [],
									h = [];
								for (e = 0; e < n.length; e++) {
									var l = n[e];
									this.enumSource[r].value ? (h[e] = this.enumSource[r].value({ i: e, item: l })) : (h[e] = n[e]),
										this.enumSource[r].title ? (a[e] = this.enumSource[r].title({ i: e, item: l })) : (a[e] = h[e]);
								}
		...
		(n.defaults.editors.multiselect = n.AbstractEditor.extend({
			preBuild: function () {
				...
				for (this.option_keys = [], a = 0; a < e.length; a++)
					this.sanitize(e[a]) === e[a] && (this.option_keys.push(e[a] + ''), (this.select_values[e[a] + ''] = e[a]));
			},
			build: function () {
				var t,
					e = this;
				...
				this.container.appendChild(this.control),
					this.control.addEventListener('change', function (i) {
						...
						for (t = 0; t < e.option_keys.length; t++)
							(e.select_options[e.option_keys[t]].selected || e.select_options[e.option_keys[t]].checked) &&
			...
			setValue: function (t, e) {
				var i;
				for (t = t || [], 'object' != typeof t ? (t = [t]) : Array.isArray(t) || (t = []), i = 0; i < t.length; i++)
					'string' != typeof t[i] && (t[i] += '');
				for (i in this.select_options)
					this.select_options.hasOwnProperty(i) &&
						(this.select_options[i]['select' === this.input_type ? 'selected' : 'checked'] = -1 !== t.indexOf(i));
			...
			getNumColumns: function () {
				...
				for (var e in this.select_values)
					this.select_values.hasOwnProperty(e) && (t = Math.max(t, (this.select_values[e] + '').length + 4));
				return Math.min(12, Math.max(t / 7, 2));
			},
			...
			enable: function () {
				if (!this.always_disabled) {
					...
					else if (this.inputs) for (var t in this.inputs) this.inputs.hasOwnProperty(t) && (this.inputs[t].disabled = !1);
		...
		(n.defaults.editors.upload = n.AbstractEditor.extend({
			...
			refreshPreview: function () {
				if (
					...
					((this.last_preview = this.preview_value), (this.preview.innerHTML = ''), this.preview_value)
				) {
					var t = this,
					...
					if (
						((this.preview.innerHTML = '<strong>Type:</strong> ' + e + ', <strong>Size:</strong> ' + i.size + ' bytes'),
						'image' === e.substr(0, 5))
					) {
						this.preview.innerHTML += '<br>';
						...
						r.addEventListener('click', function (e) {
							e.preventDefault(),
								...
								t.theme.getProgressBar &&
									((t.progressBar = t.theme.getProgressBar()), t.preview.appendChild(t.progressBar)),
								t.jsoneditor.options.upload(t.path, i, {
									success: function (e) {
										t.setValue(e),
											t.parent ? t.parent.onChildEditorChange(t) : t.jsoneditor.onChange(),
		...
		(n.defaults.editors.checkbox = n.AbstractEditor.extend({
			...
			build: function () {
				var t = this;
				this.options.compact || (this.label = this.header = this.theme.getCheckboxLabel(this.getTitle())),
					this.schema.description && (this.description = this.theme.getFormInputDescription(this.schema.description)),
					this.options.compact && (this.container.className += ' compact'),
	...
	(n.AbstractTheme = t.extend({
		...
		getHeader: function (t, e) {
			...
			return 'string' == typeof t ? (i.textContent = t) : i.appendChild(t), e && (i.className += ' required'), i;
		},
		...
		getMultiCheckboxHolder: function (t, e, i) {
			...
			for (var r in t)
				t.hasOwnProperty(r) && ((t[r].style.display = 'inline-block'), (t[r].style.marginRight = '20px'), s.appendChild(t[r]));
		...
		getFormControl: function (t, e, i) {
			...
			return (
				...
				t && s.appendChild(t),
				'checkbox' === e.type ? t.insertBefore(e, t.firstChild) : s.appendChild(e),
				i && s.appendChild(i),
				s
			);
		},
		...
		(n.defaults.themes.bootstrap2 = n.AbstractTheme.extend({
			...
			getFormInputLabel: function (t, e) {
				var i = this._super(t);
				return (i.style.display = 'inline-block'), (i.style.fontWeight = 'bold'), e && (i.className += ' required'), i;
			},
			setGridColumnSize: function (t, e) {
				t.className = 'span' + e;
			},
			...
			afterInputReady: function (t) {
				t.controlgroup ||
					...
					this.closest(t, '.compact') &&
						((t.controlgroup.className = t.controlgroup.className.replace(/control-group/g, '').replace(/[ ]{2,}/g, ' ')),
						(t.controls.className = t.controlgroup.className.replace(/controls/g, '').replace(/[ ]{2,}/g, ' ')),
			...
			getFormControl: function (t, e, i) {
				...
				return (
					...
					t && 'checkbox' === e.getAttribute('type')
						? (s.appendChild(r), (t.className += ' checkbox'), t.appendChild(e), r.appendChild(t), (r.style.height = '30px'))
						: (t && ((t.className += ' control-label'), s.appendChild(t)), r.appendChild(e), s.appendChild(r)),
					i && r.appendChild(i),
					s
				);
			},
			...
			removeInputError: function (t) {
				t.errmsg &&
					((t.errmsg.style.display = 'none'), (t.controlgroup.className = t.controlgroup.className.replace(/\s?error/g, '')));
			},
			getTabHolder: function () {
				var t = document.createElement('div');
				return (
			...
			getTab: function (t) {
				...
				return i.setAttribute('href', '#'), i.appendChild(t), e.appendChild(i), e;
			},
			getTabContentHolder: function (t) {
				return t.children[1];
			},
			...
			getProgressBar: function () {
				...
				var e = document.createElement('div');
				return (e.className = 'bar'), (e.style.width = '0%'), t.appendChild(e), t;
			},
			updateProgressBar: function (t, e) {
				t && (t.firstChild.style.width = e + '%');
			},
			updateProgressBarUnknown: function (t) {
				t && ((t.className = 'progress progress-striped active'), (t.firstChild.style.width = '100%'));
			},
		})),
		(n.defaults.themes.bootstrap3 = n.AbstractTheme.extend({
			...
			afterInputReady: function (t) {
				t.controlgroup ||
					...
					this.closest(t, '.compact') && (t.controlgroup.style.marginBottom = 0));
			},
			getTextareaInput: function () {
				var t = document.createElement('textarea');
			...
			getFormControl: function (t, e, i) {
				...
				return (
					t && 'checkbox' === e.type
						...
						: ((s.className += ' form-group'), t && ((t.className += ' control-label'), s.appendChild(t)), s.appendChild(e)),
					i && s.appendChild(i),
					s
				);
			},
			...
			removeInputError: function (t) {
				t.errmsg &&
					((t.errmsg.style.display = 'none'), (t.controlgroup.className = t.controlgroup.className.replace(/\s?has-error/g, '')));
			},
			getTabHolder: function () {
				var t = document.createElement('div');
				return (
					(t.innerHTML = "<div class='tabs list-group col-md-2'></div><div class='col-md-10'></div>"), (t.className = 'rows'), t
				);
			},
			getTab: function (t) {
				var e = document.createElement('a');
		...
		(n.defaults.themes.foundation = n.AbstractTheme.extend({
			...
			getSelectInput: function (t) {
				var e = this._super(t);
				return (e.style.minWidth = 'none'), (e.style.padding = '5px'), (e.style.marginTop = '3px'), e;
			},
			getSwitcher: function (t) {
				var e = this._super(t);
			...
			afterInputReady: function (t) {
				this.closest(t, '.compact') && (t.style.marginBottom = 0), (t.group = this.closest(t, '.form-control'));
			},
			getFormInputLabel: function (t, e) {
				var i = this._super(t);
				return (i.style.display = 'inline-block'), e && (i.className += ' required'), i;
			},
			getFormInputField: function (t) {
				var e = this._super(t);
				return (e.style.width = '100%'), (e.style.marginBottom = 'checkbox' === t ? '0' : '12px'), e;
			},
			getFormInputDescription: function (t) {
				var e = document.createElement('p');
				return (e.textContent = t), (e.style.marginTop = '-10px'), (e.style.fontStyle = 'italic'), e;
			},
			getIndentedPanel: function () {
				var t = document.createElement('div');
			...
			addInputError: function (t, e) {
				t.group &&
					...
					t.errmsg
						? (t.errmsg.style.display = '')
						: (t.insertAdjacentHTML('afterend', '<small class="error"></small>'),
						  (t.errmsg = t.parentNode.getElementsByClassName('error')[0])),
					(t.errmsg.textContent = e));
			},
			removeInputError: function (t) {
				t.errmsg && ((t.group.className = t.group.className.replace(/ error/g, '')), (t.errmsg.style.display = 'none'));
			},
			getProgressBar: function () {
				var t = document.createElement('div');
				t.className = 'progress';
				var e = document.createElement('span');
				return (e.className = 'meter'), (e.style.width = '0%'), t.appendChild(e), t;
			},
			updateProgressBar: function (t, e) {
				t && (t.firstChild.style.width = e + '%');
			},
		...
		(n.defaults.themes.foundation3 = n.defaults.themes.foundation.extend({
			...
			getFormInputLabel: function (t, e) {
				var i = this._super(t);
				return (i.style.fontWeight = 'bold'), e && (i.className += ' required'), i;
			},
			getTabHolder: function () {
				var t = document.createElement('div');
				return (
					(t.className = 'row'),
					(t.innerHTML = "<dl class='tabs vertical two columns'></dl><div class='tabs-content ten columns'></div>"),
					t
				);
			},
			setGridColumnSize: function (t, e) {
				var i = ['zero', 'one', 'two', 'three', 'four', 'five', 'six', 'seven', 'eight', 'nine', 'ten', 'eleven', 'twelve'];
			...
			getTabContent: function () {
				var t = document.createElement('div');
				return (t.className = 'content active'), (t.style.paddingLeft = '5px'), t;
			},
			markTabActive: function (t) {
				t.className += ' active';
			},
		...
		(n.defaults.themes.foundation5 = n.defaults.themes.foundation.extend({
			...
			getTabHolder: function () {
				var t = document.createElement('div');
				return (t.innerHTML = "<dl class='tabs vertical'></dl><div class='tabs-content vertical'></div>"), t;
			},
			getTab: function (t) {
				var e = document.createElement('dd'),
					i = document.createElement('a');
		...
		(n.defaults.themes.html = n.AbstractTheme.extend({
			...
			getFormInputDescription: function (t) {
				var e = this._super(t);
				return (
					(e.style.fontSize = '.8em'), (e.style.margin = 0), (e.style.display = 'inline-block'), (e.style.fontStyle = 'italic'), e
				);
			},
			getIndentedPanel: function () {
				var t = this._super();
			...
			getTable: function () {
				var t = this._super();
				return (t.style.borderBottom = '1px solid #ccc'), (t.style.marginBottom = '5px'), t;
			},
			addInputError: function (t, e) {
		...
		(n.defaults.themes.jqueryui = n.AbstractTheme.extend({
			...
			getTableHeaderCell: function (t) {
				var e = this._super(t);
				return (e.className = 'ui-state-active'), (e.style.fontWeight = 'bold'), e;
			},
			getTableCell: function () {
				var t = this._super();
			...
			getDescription: function (t) {
				var e = document.createElement('span');
				return (e.style.fontSize = '.8em'), (e.style.fontStyle = 'italic'), (e.textContent = t), e;
			},
			getButtonHolder: function () {
				var t = document.createElement('div');
			...
			getFormInputLabel: function (t, e) {
				...
				return (
					(i.style.fontWeight = 'bold'), (i.style.display = 'block'), e && (i.className += ' required'), (i.textContent = t), i
				);
			},
			getButton: function (t, e, i) {
				var s = document.createElement('button');
				...
				return (r.className = 'ui-button-text'), (r.textContent = t || i || '.'), s.appendChild(r), s.setAttribute('title', i), s;
			},
			setButtonText: function (t, e, i, s) {
				(t.innerHTML = ''),
				...
				var r = document.createElement('span');
				(r.className = 'ui-button-text'), (r.textContent = e || s || '.'), t.appendChild(r), t.setAttribute('title', s);
			},
			getIndentedPanel: function () {
				var t = document.createElement('div');
				return (
					(t.className = 'ui-widget-content ui-corner-all'), (t.style.padding = '1em 1.4em'), (t.style.marginBottom = '20px'), t
				);
			},
			afterInputReady: function (t) {
			...
			markTabActive: function (t) {
				t.className = t.className.replace(/\s*ui-widget-header/g, '') + ' ui-state-active';
			},
			markTabInactive: function (t) {
				t.className = t.className.replace(/\s*ui-state-active/g, '') + ' ui-widget-header';
			},
		})),
		(n.AbstractIconLib = t.extend({
			mapping: { collapse: '', expand: '', delete: '', edit: '', add: '', cancel: '', save: '', moveup: '', movedown: '' },
			icon_prefix: '',
		...
		(n.defaults.translate = function (t, e) {
			...
			if (e) for (var r = 0; r < e.length; r++) s = s.replace(new RegExp('\\{\\{' + r + '}}', 'g'), e[r]);
			return s;
		}),
		...
		(n.plugins = { ace: { theme: '' }, epiceditor: {}, sceditor: {}, select2: {} });
	for (var a in n.defaults.editors)
		n.defaults.editors.hasOwnProperty(a) && (n.defaults.editors[a].options = n.defaults.editors.options || {});
	n.defaults.resolvers.unshift(function (t) {
		...
		n.defaults.resolvers.unshift(function (t) {
			return 'boolean' === t.type ? ('checkbox' === t.format || (t.options && t.options.checkbox) ? 'checkbox' : 'select') : void 0;
		}),
		n.defaults.resolvers.unshift(function (t) {
		...
		n.defaults.resolvers.unshift(function (t) {
			return 'string' === t.type && 'url' === t.format && t.options && t.options.upload === !0 && window.FileReader
				? 'upload'
				: void 0;
		}),
		n.defaults.resolvers.unshift(function (t) {
		...
		n.defaults.resolvers.unshift(function (t) {
			if (t['enum']) {
				...
				if ('number' === t.type || 'integer' === t.type || 'string' === t.type) return 'select';
			}
		}),
		n.defaults.resolvers.unshift(function (t) {
			return 'array' === t.type &&
			...
			if (window.jQuery || window.Zepto) {
				...
				(t.jsoneditor = n.defaults),
					(t.fn.jsoneditor = function (t) {
						...
						if ('value' === t) {
							if (!i) throw 'Must initialize jsoneditor before getting/setting the value';
