...
Sys._ScriptLoader._errorScriptLoadFailed = function (b) {
	...
	var d = 'Sys.ScriptLoadFailedException: ' + String.format(a, b),
		c = Error.create(d, { name: 'Sys.ScriptLoadFailedException', scriptUrl: b });
	c.popStackFrame();
	return c;
};
...
Sys.WebForms.PageRequestManager.prototype = {
	...
	_createPageRequestManagerTimeoutError: function () {
		var b = 'Sys.WebForms.PageRequestManagerTimeoutException: ' + Sys.WebForms.Res.PRM_TimeoutError,
			a = Error.create(b, { name: 'Sys.WebForms.PageRequestManagerTimeoutException' });
		a.popStackFrame();
		return a;
	},
	_createPageRequestManagerServerError: function (a, d) {
		var c = 'Sys.WebForms.PageRequestManagerServerErrorException: ' + (d || String.format(Sys.WebForms.Res.PRM_ServerError, a)),
			b = Error.create(c, { name: 'Sys.WebForms.PageRequestManagerServerErrorException', httpstatusCode: a });
		b.popStackFrame();
		return b;
	},
	_createPageRequestManagerParserError: function (b) {
		var c = 'Sys.WebForms.PageRequestManagerParserErrorException: ' + String.format(Sys.WebForms.Res.PRM_ParserError, b),
			a = Error.create(c, { name: 'Sys.WebForms.PageRequestManagerParserErrorException' });
		a.popStackFrame();
		return a;
	},
	...
	_onFormSubmit: function (i) {
		...
		for (f = 0; f < x; f++) {
			...
			if (n === 'INPUT') {
				var l = e.type;
				if (l === 'text' || l === 'password' || l === 'hidden' || ((l === 'checkbox' || l === 'radio') && e.checked)) {
		...
		if (Sys.Browser.agent === Sys.Browser.InternetExplorer) {
			...
			if (m !== -1) {
				var t = c.substr(0, m);
				if (t.indexOf('%') === -1) c = encodeURI(t) + c.substr(m);
	...
	_onFormSubmitCompleted: function (c) {
		...
		k.loadScripts(
			0,
			Function.createDelegate(this, Function.createCallback(this._scriptIncludesLoadComplete, a)),
			Function.createDelegate(this, Function.createCallback(this._scriptIncludesLoadFailed, a)),
			null,
		);
	},
	...
	_parseDelta: function (h) {
		...
		while (b < c.length) {
			...
			b++;
			Array.add(j, { type: E, id: F, content: D });
		}
		if (e) {
			this._endPostBack(
				this._createPageRequestManagerParserError(String.format(Sys.WebForms.Res.PRM_ParserErrorDetails, e)),
				h,
				null,
			);
			return null;
		...
		for (var k = 0, G = j.length; k < G; k++) {
			...
			switch (a.type) {
				...
				default:
					this._endPostBack(
						this._createPageRequestManagerParserError(String.format(Sys.WebForms.Res.PRM_UnknownToken, a.type)),
						h,
						null,
					);
					return null;
...
Sys.WebForms.Res = {
	...
	PRM_ParserError:
		'The message received from the server could not be parsed. Common causes for this error are when the response is modified by calls to Response.Write(), response filters, httpsModules, or server trace is enabled.\r\nDetails: {0}',
