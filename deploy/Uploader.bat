...
::Variables
  ...
  set build=Release
  set build=Release
  set /p uploadType="Enter Upload Type (C,R,S,A): "
cls
...
:DEPLOY_C
  ...
  set CDNDir=C:\dev\web\mfdlabs.com\pages\setup\build\%typeName%\
  rmdir %CDNDir% /s /q
  mkdir %CDNDir%
...
:DEPLOY_R
  ...
  GOTO :EOF
:DEPLOY_S
  call :ColorText 09 "Deploying studio..."
  set typeName=RobloxStudio
  set CDNDir=C:\dev\web\mfdlabs.com\pages\setup\build\%typeName%\
  rmdir %CDNDir% /s /q
  mkdir %CDNDir%
  cd "C:\dev\roblox\UploadBits\%platform%-%build%-%typeName%"\
  CALL :DEPLOY
  call :ColorText 02 "Finished deploying studio!"
  GOTO :EOF
:DEPLOY_A
  call :ColorText 09 "Deploying all..."
  CALL :DEPLOY_C
  CALL :DEPLOY_R
  CALL :DEPLOY_S
  call :ColorText 02 "Finished deploying all!"
  GOTO :EOF
:UNKNOWN_DEPLOY
...
:DEPLOY
  ...
  for /f "tokens=*" %%f in ('dir /b *') do (
    ...
    >> "%CDNDir%%%f" rem/
    xcopy /Q /Y /F "..\%platform%-%build%-%typeName% "%CDNDir%%%f" > nul
