        ...
        controllers.forEach((v) => {
            if (!v.includes('.js.map') || !v.includes('.d.ts')) {
                ...
                if (map.default) {
                    ...
                    try {
                        if (opts.logSetups)
                            console.log(`Mapping GET wss://${opts.apiName + dir}`);
                        const wsServer = new ws_1.default.Server({ path: dir, port: 80, server: HttpServer });
                        const wssServer = new ws_1.default.Server({ path: dir, port: 443, server: HttpsServer });
